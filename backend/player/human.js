...
module.exports = class Human extends Player {
  ...
  confirmSelection() {
    ...
    */
    if (!pack || typeof pack.length === "undefined") return;
    // End of workaround
    ...
    remove(pack, (card) => {
      logger.info(
        `GameID: ${this.GameId}, player ${this.name}, burnt: ${card.name}`
      );
      return this.selected.burns.includes(card.cardId);
    });
  ...
  handleTimeout() {
    ...
    // pick cards
    const remainingToPick = Math.min(
      pack.length,
      this.picksPerPack - this.selected.picks.length
    );
